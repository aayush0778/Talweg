...
Trains a Scikit-Learn RandomForestRegressor surrogate model on systematically generated
...
def generate_training_data(n_random: int = 60_000, random_seed: int = 42) -> tuple[np.ndarray, np.ndarray]:
    """Generates synthetic feature matrices including boundary grid and random uniform samples."""
    np.random.seed(random_seed)

    # 1. Systematic grid sampling covering corners, midpoints, and extremes
    r24_grid = np.linspace(0.0, NORMALIZATION_MAX["rainfall_24h"], 7)
    r3d_grid = np.linspace(0.0, NORMALIZATION_MAX["rainfall_3d"], 7)
    sm_grid = np.linspace(0.0, NORMALIZATION_MAX["soil_moisture"], 7)
    slope_grid = np.linspace(0.0, NORMALIZATION_MAX["slope"], 7)
    hd_grid = np.linspace(0.0, NORMALIZATION_MAX["historical_density"], 7)

    grid_mesh = np.meshgrid(r24_grid, r3d_grid, sm_grid, slope_grid, hd_grid)
    X_grid = np.column_stack([m.ravel() for m in grid_mesh])  # 7^5 = 16,807 samples
...
def train_surrogate_model(output_path: str | None = None) -> dict:
    """Fits RandomForestRegressor and serializes model bundle."""
    ...
    X, y = generate_training_data(n_random=60_000, random_seed=42)
    ...
    print(f"[train] Fitting RandomForestRegressor on {len(X_train)} samples...")
    ...
    assert r2 > 0.99, f"Model quality assertion failed: R²={r2} <= 0.99"
    ...
    artifact = {
        ...
        "model_type": "RandomForestRegressor (Surrogate)",
        "version": "0.1.0",
    }
